    ...
    mysql -u root -p$DBPASS -e "USE $DBNAME; CREATE TABLE Diapositives (ID_Diapositiva INT AUTO_INCREMENT PRIMARY KEY, titol VARCHAR(255) NOT NULL, contingut TEXT, orden INT NOT NULL, ID_Presentacio INT,CONSTRAINT UQ_Diapositiva UNIQUE (orden), FOREIGN KEY (ID_Presentacio) REFERENCES Presentacions(ID_Presentacio));"
